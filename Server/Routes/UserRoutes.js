...
router.post(
  ...
  async (req, res) => {
    ...
    const profilePicture = req.file ? `/uploads/${req.file.filename}` : ""; // URL path for the uploaded file
...
router.post("/addChat/:username", async (req, res) => {
  ...
  try {
    ...
    if (newChatUsername === username) {
      return res.status(400).json({ message: "You cannot add yourself to chats" });
    }
